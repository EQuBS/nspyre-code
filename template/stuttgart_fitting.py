...
def LorentzianEstimator(x, y):
	y = np.asarray(y) # Modification Rolando 9/17/2025
	# Handling old and new scipy.stats.mode
	mode_result = scipy.stats.mode(y)
	if hasattr(mode_result, 'mode'):
		# New Version of scipy
		c = mode_result.mode[0] if np.ndim(mode_result.mode) > 0 else mode_result.mode
	else:
		# Old Version of scipy
		try:
			c = mode_result[0][0]
		except (IndexError, TypeError):
			c = float(mode_result[0]) if np.ndim(mode_result[0]) == 0 else mode_result[0][0]
...
def fit_multiple_lorentzians(x,y,number_of_lorentzians='auto',threshold=0.5):
	...
	if number_of_lorentzians == 'auto': # try to find N automatically
...
if __name__ == '__main__':
	...
	#d = pickle.load(open('2012-01-25_tsukubac12_nv39_pulsed_precise_R_ODMR.pys','rb'))
