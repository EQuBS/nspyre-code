...
class DLnsec():
    ...
    def las_mode(self):
        """Put the laser into LAS mode. This isn't a real CW mode, because a low
        on the trigger input will still turn the lasing off."""
        self._send('LAS')
        _logger.info(f'Setting {self} to LAS mode.')

    def int_mode(self):
        """Put the laser into internal mode. Don't really know what it is."""
        self._send('INT')
