...
class Pulses():
    ...
    def __init__(self):
        ...
        # 20231207 calibrated by multimeter and PulseStreamer controller
    ...
    def Pulsed_ODMR(self, pi_xy, pi_time, runs):
        '''
    ...
    def Rabi(self, params, pi_xy, init_time, read_time, wait_time, read_wait):
        '''
        Rabi sequence
    ...
    def Rabi_WindFreak(self, params, pi_xy, init_time, read_time, wait_time, read_wait):
        '''
    ...
    def Calibrate_LaserLag(self, params, buffer_time, read_window, laser_window):
        ...
        longest_time = self.convert_type(round(params[-1]), float) + laser_window + self.rest_time_btw_seqs
    ...
    def Optical_T1(self, params, tau_balance, init_time, read_time, forNV=True):
        '''
        ...
        def SingleOptical_T1(tau_time, tau_balance, forNV=True):
            '''
            CREATE SINGLE T1 SEQUENCE TO REPEAT THROUGHOUT EXPERIMENT
            '''
        ...
        seqs_total_time = 0
        for tau in params:
            seqs += SingleOptical_T1(tau, tau_balance, forNV)
    ...
    def Diff_T1(self, params, tau_balance, pi_xy, pi_time, init_time, read_time):#the wait time here is used to replace the self.singlet_decay
        '''
    ...
    def Diff_T1rho(self, params, tau_balance, pihalf_y, init_time, read_time):
        '''
    ...
    def Diff_T1_Switch(self, params, tau_balance, pi_xy, pi_time, init_time, read_time):
        '''
    ...
    def Ramsey(self, params, tau_balance, pihalf_x, pihalf_y):
    ...
    def Echo(self, params, tau_balance, pihalf_x, pihalf_y, pi_x, pi_y, init_time, read_time, wait_time, read_wait):
        '''
    ...
    def DEER(self, pihalf_x, pihalf_y, pi_x, pi_y, tau, pi_electron):
        '''
        DEER pulse sequence.
        MW sequence: pi/2(x) - tau - pi(y) - tau - pi/2(x)
    ...
    def DEER_Rabi(self, params, pihalf_x, pihalf_y, pi_x, pi_y, tau):
        '''
        DEER pulse sequence.
        MW sequence: pi/2(x) - tau - pi(y) - tau - pi/2(x)
    ...
    def DEER_FID(self, params, pihalf_x, pihalf_y, pi_x, pi_y, pi_electron):
        ...
        def SingleDEERFID(precession_tau):
            ...
            laser_off1 = self.singlet_decay + pihalf_x + precession_tau + pi_y + precession_tau + pihalf_x + self.MW_buffer_time
            laser_off2 = 1000
            self.total_time = self.laser_time + laser_off1 + self.readout_time + laser_off2
            # DAQ trigger windows
            clock_off1 = self.laser_time + laser_off1 + self.laser_lag
    ...
    def DEER_Correlation_Rabi(self, params, pihalf_x, pihalf_y, pi_x, pi_y, tau0, middle_pulse_start_time, tau, pi_electron):
            ...
            def SingleCorrelationRabi(e_pulse_time):
                ...
                #Tian-Xing: The pulse sequence programming principle is that we firstly write down the total time we need for laser, without considering self.laser_lag. Then all the timing for other devices are shifted by 1*self.laser_lag. And the total sequence time for each device are always the same.
                #tau0= #a number needs to fill here
                laser_off1 = self.singlet_decay + pihalf_x + tau0 + pi_y + tau0 + pihalf_x + self.MW_buffer_time + tau + pihalf_x + tau0 + pi_y + tau0 + pihalf_x
                laser_off2 = 1000
                self.total_time = self.laser_time + laser_off1 + self.readout_time + laser_off2
                # DAQ trigger windows
                clock_off1 = self.laser_time + laser_off1 + self.laser_lag
    ...
    def ReporterSpin_T2(self, params, pihalf_x, pihalf_y, pi_x, pi_y, tau0, spin_echo_first_DEER_gap_time, tau, pi_electron, pi_half_electron):
