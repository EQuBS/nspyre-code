"""
    ...
    Modifications: Nazar, Dec 2020, cleaned; 
                   Chris, Dec 2022, de-lantz-ed and simplified by assuming all voltages in V, pos/dist's in um, and acq_rates in Hz.
                   Tian-Xing Zheng, Sept 2023, changed function name __enter__ to initialize, __exit__ to finalize
...
class NIDAQAxis():
    ...
    def __init__(self, ao_ch, cal, limits=(None, None)):#, units='um'):
        ...
        Arguments:  *ao_ch: NI DAQ channel string e.g. 'Dev/ao0'
                    ...
                    *limits: Limits tuple in um e.g. (0, 1e5) for 0 to 10cm. Default: (None,None). The Voltage limits are -10V, +10V
        """
    ...
    def um_to_volts(self, pos):
        ...
        if not isinstance(pos, (int, float)):
            pos = float(pos)  # Convert rpyc or other non-native types to a float
        #result = pos * self.cal
        return (pos * self.cal)
...
class NIDAQMotionController():
    ...
    def new_ctr_task(self, ctr_ch):
        ...
        #TXZ: we don't need to define edge, initial_count, count_direction here because now the reading is not riggered by the clk sent from the pulse streamer
        ...
        self.current_counter_task.ci_channels.all.ci_count_edges_term = '/Dev1/PFI3' #Hard-coded for now
        #print('\n ci_count_edge_term finished')
        self.counter_tasks.append(self.current_counter_task)
        #print('\n self.counter_tasks.append finished')
    ...
    def line_scan(self, init_point, final_point, steps, pts_per_step=1):
        ...
        #print(init_point)
        #print('(ni_motion_controller) final_point:',final_point)
        #print('(ni_motion_controller) type final_point:',type(final_point))
        #print('(ni_motion_controller) type final_point[x]:',type(final_point['x']))
        
        self.move(init_point)
        step_voltages = self.linear_func(init_point, final_point, steps)
        ## repeat pts_per_step times for each scanning point
        ...
        device_name = 'Dev1' # TXZ: Hard Code for now
        #print('\n device name:', device_name)
        #Create a new counter input task
        #print('ready to do new_ctr_task()')
        #print('self.ctr_ch:',self.ctr_ch)
        self.new_ctr_task(self.ctr_ch)
        #print('finish new_ctr_task()')
        ...
        #print('\n finish current_counter_task.timing.cfg_samp_clk_timing')
        ...
        #print('\n finish current_counter_task.triggers.arm_start_trigger.dig_edge_src')
        ...
        ## Do the math for the data_buffer that we get from NI-DAQ counter.
        ...
        #self.__enter__()
        ## convert the fluorescence counts to count/s
    ...
    def smooth_func(self, start_pt, stop_pt):
        ...
        #import pdb; pdb.set_trace()
        #print("smooth_func called with:")
        #print(f"start_pt: {start_pt}, stop_pt: {stop_pt}")

        start_volts = np.array([])
        stop_volts = np.array([])
        for axis in self.axes:
            # print('start_pt[axis] = ', start_pt[axis])
            # print('type start_pt[axis]:',type(start_pt[axis]))
            # print('self.axes[axis].um_to_volts(start_pt[axis]) = ', self.axes[axis].um_to_volts(start_pt[axis]))
            # print('tyep of the voltage = ', type(self.axes[axis].um_to_volts(start_pt[axis])))

            # print('stop_pt[axis] = ', stop_pt[axis])
            # print('type stop_pt[axis]:',type(stop_pt[axis]))
            # print('self.axes[axis].um_to_volts(stop_pt[axis]) = ', self.axes[axis].um_to_volts(stop_pt[axis]))
            # print('tyep of the voltage = ', type(self.axes[axis].um_to_volts(stop_pt[axis])))
            # Debug prints
            #print(f"start_pt[{axis}] = {start_pt[axis]}, type: {type(start_pt[axis])}")
            #print(f"stop_pt[{axis}] = {stop_pt[axis]}, type: {type(stop_pt[axis])}")
            #start_volt = self.axes[axis].um_to_volts(start_pt[axis])
            #stop_volt = self.axes[axis].um_to_volts(stop_pt[axis])
            # Debug prints
            #start_volts = np.append(start_volts, self.axes[axis].um_to_volts(start_pt[axis])) #Commented out on 11/13/2024 for correcting the data type 
            #stop_volts = np.append(stop_volts, self.axes[axis].um_to_volts(stop_pt[axis]))  #Commented out on 11/13/2024 for correcting the data type 

            start_volts = np.append(start_volts, float(self.axes[axis].um_to_volts(start_pt[axis])))
            stop_volts = np.append(stop_volts, float(self.axes[axis].um_to_volts(stop_pt[axis])))
