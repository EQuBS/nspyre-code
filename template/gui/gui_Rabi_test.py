...
class RabiWidget(ExperimentWidget):
    def __init__(self):
        ...
        params_config = {
            ...
            'rabi_type':{
                'display_text': 'Rabi Type: ',
                'widget': self.rabi_type_combo,
            },
        }
...
def process_Rabi_data(sink: DataSink):
    ...
    for s,_ in enumerate(sink.datasets['signal']):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            #contrast2 = (bg - sig) / (bg + sig)
            #contrast2 = np.where(np.isfinite(contrast), contrast2, np.nan)
            norm_diff = (sig - bg) / bg
            norm_diff = np.where(np.isfinite(norm_diff), norm_diff, np.nan)

        contrast_sweeps.append(np.stack([mw_times, contrast]))
        #contrast2_sweeps.append(np.stack([mw_times, contrast2]))
    ...
    #sink.datasets['contrast2'] = contrast2_sweeps
    ...
    if sink.datasets['signal'] and sink.datasets['background']:
        ...
        sink.datasets['contrast_from_avg2'] = [
            np.stack([mw_times, contrast_from_avg2])
        ]
...
class FlexLinePlotWidgetWithRabi(FlexLinePlotWidget):
    ...
    def __init__(self):
        ...
        #2026-5-25 - Append
        self.add_plot('sig_avg_append',        series='signal',   scan_i='',     scan_j='',  processing='Append')
        self.add_plot('bg_avg_append',         series='background',   scan_i='',     scan_j='',  processing='Append')
        self.hide_plot('sig_avg_append')
        self.hide_plot('bg_avg_append')
        ...
        self.add_plot(
            'contrast_from_avg2',
            series='contrast_from_avg2',
            scan_i='',
            scan_j='',
            processing='Average'
        )
        self.hide_plot('contrast_from_avg2')
        self.hide_plot('contrast_from_avg2')
